...
interface Database {
    ...
    suspend fun getPodcastById(browseId: String): PodcastEntity?

    @Query("SELECT * FROM podcasts ORDER BY lastUpdated DESC")
    fun getAllPodcasts(): PagingSource<Int, PodcastEntity>

    @Query("SELECT * FROM podcasts WHERE browseId IN (SELECT browseId FROM podcast_subscriptions) ORDER BY lastUpdated DESC")
    ...
    @Query("UPDATE podcast_episodes SET isCompleted = :isCompleted WHERE videoId = :videoId")
    suspend fun markEpisodeCompleted(videoId: String, isCompleted: Boolean)

    @Query("UPDATE podcast_episodes SET isDownloaded = :isDownloaded, downloadPath = :downloadPath WHERE videoId = :videoId")
    ...
    @Query("DELETE FROM podcast_subscriptions WHERE browseId = :browseId")
    suspend fun unsubscribeFromPodcast(browseId: String)

    @Query("SELECT EXISTS(SELECT 1 FROM podcast_subscriptions WHERE browseId = :browseId)")
    fun isSubscribed(browseId: String): Flow<Boolean>
...
abstract class DatabaseInitializer protected constructor() : RoomDatabase() {
    ...
    class From30To31Migration : Migration(30, 31) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `podcast_episodes` (
                        ...
                        `playPositionMs` INTEGER NOT NULL,
                        `durationMs` INTEGER,
                        ...
                        `customMetadata` TEXT,
                        PRIMARY KEY(`videoId`),
                        FOREIGN KEY(`podcastId`) REFERENCES `podcasts`(`browseId`) ON DELETE CASCADE
                    )
                """.trimIndent()
            )
            ...
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_podcast_episodes_podcastId` ON `podcast_episodes` (`podcastId`)
                """.trimIndent()
            )
