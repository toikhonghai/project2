...
import kotlinx.serialization.json.Json
 // đây là một thư viện JSON cho Kotlin, giúp phân tích cú pháp và tạo JSON một cách dễ dàng.
object Translate {
    internal val client by lazy { // đây là một đối tượng HttpClient được khởi tạo một lần duy nhất và được sử dụng lại trong toàn bộ ứng dụng
        HttpClient(CIO) {
            ...
            install(HttpTimeout) { // cài đặt plugin HttpTimeout để quản lý thời gian chờ cho các yêu cầu HTTP
            ...
            install(UserAgent) { // cài đặt plugin UserAgent để thêm thông tin về trình duyệt vào các yêu cầu HTTP
                agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" // đây là một chuỗi đại diện cho trình duyệt và hệ điều hành
            }
        }
    }

    internal val dt = listOf("at", "bd", "ex", "ld", "md", "qca", "rw", "rm", "ss", "t") // đây là danh sách các loại dữ liệu mà Google Translate hỗ trợ

    // đây là danh sách các loại dữ liệu mà Google Translate hỗ trợ
    @Suppress("unused")
    val hostNames by lazy {
        listOf( // đây là danh sách các tên miền của Google Translate
